...
from load_models.aux_functions import load_Condensate
...
for model in models:
    print(files_CLN_500m_1h)
    TC_CLN[model],TCL_CLN[model],TCI_CLN[model]=load_Condensate(files_CLN_500m_1h[model],model)
    TC_POL[model],TCL_POL[model],TCI_POL[model]=load_Condensate(files_POL_500m_1h[model],model)
    # TC_CLN[model]=load_Condensate(files_CLN_500m_1h[model],model)
    # TC_POL[model]=load_Condensate(files_CLN_500m_1h[model],model)
...
for model,TCL_i in TCL_POL.items():  
    ...
    print('Liquid water profile calculated and plotted for',model)
...
for i,model in enumerate(models):
    ...
    plt.ylim((0,15))
    print('TCL diff  vs. Height calculated and plotted for',model)
...
############################################################
####### Plot Mean Condensate Profile Total for entire simulation
############################################################
matplotlib.rcParams.update({'font.size': 14})
fig1,ax1=plt.subplots(figsize=(10,6),nrows=1,ncols=1)
cnt = 0
for model,TC_i in TC_CLN.items():  
    yaxis = TC_i.coord('geopotential_height').points
    data = TC_i.collapsed(('x','y','time'),MEAN).data

    plt.plot(data,yaxis/1000,color=color[model],linestyle='--',label=model+' CLN')
    
for model,TC_i in TC_POL.items():  
    yaxis = TC_i.coord('geopotential_height').points
    data = TC_i.collapsed(('x','y','time'),MEAN).data
    plt.plot(data,yaxis/1000,color=color[model],linestyle='-',label=model+' POL')
    print('TC profile calculated and plotted for',model)

plt.ylim((0,15))
plt.legend()
plt.ylabel('Altitude (km)')
plt.xlabel('Condensate mixing ratio (kg/kg)')
plt.grid()
ax1.ticklabel_format(style='sci',axis='x', scilimits=(0,0))
plt.tight_layout()
os.makedirs('Plots/Condensate',exist_ok=True)
plt.savefig(os.path.join('Plots','Condensate',savename+'TC_prof.png'))
plt.close(fig1)


############################################################
####### Plot Mean Condensate Diff Profile Total for entire simulation
############################################################
matplotlib.rcParams.update({'font.size': 14})
fig1,ax1=plt.subplots(figsize=(6,4),nrows=1,ncols=1)
cnt = 0
for i,model in enumerate(models):
    ...
    data = TC_POL[model].collapsed(('x','y','time'),MEAN).data-TC_CLN[model].collapsed(('x','y','time'),MEAN).data

    plt.plot(data,yaxis/1000,color=color[model],linestyle='--',label=model+'POL-CLN')
    plt.ylim((0,15))
    print('TC diff profile calculated and plotted for',model)
...
os.makedirs('Plots/Condensate',exist_ok=True)
plt.savefig(os.path.join('Plots','Condensate',savename+'TC_prof_diff.png'))
plt.close(fig1)
