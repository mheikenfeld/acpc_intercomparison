...
for case in cases:
    for model in models:
        files[case]['500m']['1h'][model]=glob.glob(os.path.join(directory[case]['500m']['1h'][model],filename['500m'][model]))
        print(files[case]['500m']['1h'][model])


OLR=defaultdict(f)
for case in cases:
    for model in models:
        print(model,files[case]['500m']['1h'][model])
...
for case in cases:
    os.makedirs(os.path.join('Plots','OLR_Maps',case,'All'),exist_ok=True)
    for time in times:
        ...
        for i,model in enumerate(models):
            ...
            os.makedirs(os.path.join('Plots','OLR_Maps',case,model),exist_ok=True)
            OLR_i=OLR[case][model].extract(constraint_time)
            if OLR_i is not None:
                plot_2D_map(OLR_i,title=model,axes_extent=[-96.3,-93.9,28.4,30.5],axes=ax3_flat[i],vmin=100,vmax=300,n_levels=50,cmap='viridis')
                plot_2D_map(OLR_i,title=model,axes_extent=[-96.3,-93.9,28.4,30.5],axes=ax4,vmin=100,vmax=300,n_levels=50,cmap='viridis')
                fig4.savefig(os.path.join('Plots','OLR_Maps',case,model,'OLR_Maps'+time.strftime('%Y-%m-%d %H:%M:%S')+'.png'),dpi=600)
        fig3.savefig(os.path.join('Plots','OLR_Maps',case,'OLR_Maps'+time.strftime('%Y-%m-%d %H:%M:%S')+'.png'),dpi=600)
        plt.close()
print('done')
